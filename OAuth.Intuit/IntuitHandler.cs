...
{
    internal class IntuitHandler : RemoteAuthenticationHandler<IntuitOptions>
    {
        private static readonly DateTime Epoch = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
        private const string StateCookie = "__IntuitState";
        private const string RequestTokenEndpoint = "https://oauth.intuit.com/oauth/v1/get_request_token";
        private const string AuthenticationEndpoint = "https://appcenter.intuit.com/Connect/Begin?oauth_token=";
        private const string AccessTokenEndpoint = "https://oauth.intuit.com/oauth/v1/get_access_token";

        private readonly HttpClient _httpClient;

        public IntuitHandler(HttpClient httpClient)
        {
        ...
        protected override async Task<AuthenticateResult> HandleRemoteAuthenticateAsync()
        {
            ...
            if (StringValues.IsNullOrEmpty(realmId))
            {
                return AuthenticateResult.Fail("Missing or blank realmId");
            }
            ...
            {
                new Claim("urn:intuit:realmId", realmId, ClaimValueTypes.String, Options.ClaimsIssuer),
                new Claim("urn:intuit:requestToken", requestToken.Token, ClaimValueTypes.String, Options.ClaimsIssuer),
                new Claim("urn:intuit:requestTokenSecret", requestToken.TokenSecret, ClaimValueTypes.String, Options.ClaimsIssuer),
                new Claim("urn:intuit:accessToken", accessToken.Token, ClaimValueTypes.String, Options.ClaimsIssuer),
                new Claim("urn:intuit:accessTokenSecret", accessToken.TokenSecret, ClaimValueTypes.String, Options.ClaimsIssuer),
            },
        ...
        {
            var context = new IntuitCreatingTicketContext(Context, Options, token.UserId, token.ScreenName, token.Token, token.TokenSecret, user)
            {
        ...
        protected override async Task<bool> HandleUnauthorizedAsync(ChallengeContext context)
        {
            ...
            var redirectContext = new IntuitRedirectToAuthorizationEndpointContext(
                Context, Options,
        ...
        private async Task<RequestToken> ObtainRequestTokenAsync(string callBackUri, AuthenticationProperties properties)
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Get, RequestTokenEndpoint);
        ...
        private async Task<AccessToken> ObtainAccessTokenAsync(RequestToken token, string verifier)
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Get, AccessTokenEndpoint);
            request.Headers.Add("Authorization", authorizationHeaderBuilder.ToString());
